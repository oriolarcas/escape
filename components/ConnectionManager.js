...
/**
  * This class manages the active connections in the Escape system.
  *
  * The pattern is this:
  *
  ...
  * and to the local browser.  If the connection is successful (ie: no 
  * network errors and the certificates check out), this ChromeWorker should 
  * return with two sockets -- one established SSL socket to the local browser, 
  * and one established SSL socket to the destination server.
  *
  ...
  * ShuffleWorker is poll()ing on the ListenSocket, to accept outbound 
...
function ConnectionManager(listenSocket, nssFile, sslFile, nsprFile, 
    certificateManager, settingsManager){
...
ConnectionManager.prototype.setNextSNI = function(sni) {
  this.sni = sni;
};

ConnectionManager.prototype.setEncapsulation = function(encapsulation) {
  this.encapsulation = encapsulation;
};

ConnectionManager.prototype.setException = function(exception) {
  this.exception = exception;
};

ConnectionManager.prototype.setDirectIP = function(ip) {
  this.directIP = ip;
};
...
ConnectionManager.prototype.spawnConnection = function(clientSocket) {
  ...
  if (this.workerFactory !== null) {
    worker = this.workerFactory.newChromeWorker(
        'chrome://escape/content/workers/ConnectionWorker.js');
  } else {
  ...
  worker.onmessage = function(event) {
    ...
    NSPR.lib.PR_Write(connectionManager.wakeupWrite, 
        connectionManager.buffer, 5);
    connectionManager.certificateManager.setAnOriginalCert(event.data.host, 
        event.data.cert);
  };
  ...
  worker.postMessage({
      ...
      'isCustomSelected' : self.settingsManager.isCustomSelected(),
      'customDNS' : self.settingsManager.getCustomDNS(),
      'directIP' : self.directIP
    });
...
ConnectionManager.prototype.initializeShuffleWorker = function() {
  ...
  if (this.workerFactory !== null) {
    shuffleWorker = this.workerFactory
      .newChromeWorker('chrome://escape/content/workers/ShuffleWorker.js');
  } else {
    shuffleWorker = new ChromeWorker(
        'chrome://escape/content/workers/ShuffleWorker.js');
  }

  shuffleWorker.onmessage = function(event) {
    CV9BLog.worker('ShuffleWorker accepted connection: ' + 
