...
NSPR.initialize = function(nsprPath) {
  ...
  try {
    ...
    try {
      CV9BLog.core('Failed to find nspr4 in installed directory, ' +
          'checking system paths.');
      sharedLib = ctypes.open(ctypes.libraryName('nspr4'));
    } catch (err) {
      CV9BLog.core('Failed to find nspr4 in system paths, trying explicit ' + 
          'FreeBSD path.');
  ...
  NSPR.types.PRStartFunction = ctypes.FunctionType(ctypes.default_abi, 
      ctypes.void_t, [ctypes.voidptr_t]);

  NSPR.types.PRTimeParamFn = ctypes.FunctionType(ctypes.default_abi, 
      NSPR.types.PRTimeParameters, [NSPR.types.PRExplodedTime.ptr]);
  ...
  NSPR.lib = {
    ...
    PR_StringToNetAddr : sharedLib.declare('PR_StringToNetAddr',
                                ctypes.default_abi,
                                ctypes.int,
                                ctypes.char.ptr,
                                NSPR.types.PRNetAddr.ptr),
    ...
    PR_OpenTCPSocket : sharedLib.declare('PR_OpenTCPSocket',
                                    ctypes.default_abi,
                                    NSPR.types.PRFileDesc.ptr,
                                    ctypes.int32_t),

    PR_OpenUDPSocket : sharedLib.declare('PR_OpenUDPSocket',
                                    ctypes.default_abi,
                                    NSPR.types.PRFileDesc.ptr,
                                    ctypes.int32_t), 
    ...
    PR_RecvFrom : sharedLib.declare('PR_RecvFrom',
                          ctypes.default_abi,
                          ctypes.int32_t, // return
                          NSPR.types.PRFileDesc.ptr, // socket
                          ctypes.voidptr_t, // buffer
    ...
    PR_SendTo : sharedLib.declare('PR_SendTo',
                          ctypes.default_abi,
                          ctypes.int32_t, // return
                          NSPR.types.PRFileDesc.ptr, // socket
                          ctypes.voidptr_t, // buffer
