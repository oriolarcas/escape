...
SSL.initialize = function(sslPath) {
  ...
  try {
    ...
    CV9BLog.core('Failed to find ssl3 in installed directory, ' +
  ...
  SSL.lib = {
    ...
    SSL_ConfigServerSessionIDCache : sharedLib.declare(
    ...
    SSL_ForceHandshakeWithTimeout : sharedLib.declare(
  ...
  // Change these in about:config of firefox under security.tls.version, 
  //  if necessary
  ...
  // var status = SSL.lib.SSL_VersionRangeSetDefault(
  //  SSL.lib.ssl_variant_stream, ver_range.address());
  // if (status == -1) {
  // ver_range.max = SSL.lib.SSL_LIBRARY_VERSION_TLS_1_1; // nss versions < 3.15
  // status = SSL.lib.SSL_VersionRangeSetDefault(SSL.lib.ssl_variant_stream, 
  //  ver_range.address());
