...
function CertificateManager(serialized) {
  ...
  this.caCertificate = NSS.lib.CERT_FindCertByNickname(
    NSS.lib.CERT_GetDefaultCertDB(), 'Escape' );
  ...
  if (!this.caCertificate.isNull()) {
    ...
    var keys = this.generateKeyPair(true, 'Escape Local Private');
    this.caKey = keys.privateKey;
    this.caCertificate = this.generateCaCertificate(keys.privateKey, 
        keys.publicKey);
  ...
  this.peerKeys = this.generateKeyPair(false, 'Escape Peer Private');
}

CertificateManager.prototype.getOriginalCerts = function() {
  return this.originalCerts;
};

CertificateManager.prototype.setAnOriginalCert = function(host, cert) {
  this.originalCerts[host] = cert;
};

CertificateManager.prototype.getAnOriginalCert = function(host) {
  return this.originalCerts[host];
};
...
CertificateManager.prototype.signCertificate = function(certificate) {
  var algorithmId = NSS.lib.SEC_GetSignatureAlgorithmOidTag(
      this.caKey.contents.keyType, 191);
...
CertificateManager.prototype.importCertificate = function(certificate) {
  ...
  NSS.lib.CERT_ImportCerts(certdb, 3, 1, derCertPtr, results.address(), 1, 1, 
      'Escape');
...
CertificateManager.prototype.generateCaCertificate = function(privateKey, 
    publicKey) {
  return this.generateCertificate( privateKey, publicKey,
    'CN=Escape Local CA,OU=Escape,O=Escape,C=US',
    'CN=Escape Local CA,OU=Escape,O=Escape,C=US',
    true, null, null );
};

CertificateManager.prototype.generatePeerCertificate = function(
    certificateInfo) {
  ...
  var certificateName = 'CN=' +commonName+ ',OU=Escape,O=Escape,C=US';
...
CertificateManager.prototype.addCaExtensions = function(certificate, 
    extensionHandle) {
  var basicConstraint = NSS.types.CERTBasicConstraints({'isCA' : 1, 
    'pathLenConstraint' : 3});
  var encodedValue = NSS.types.SECItem();

  NSS.lib.CERT_EncodeBasicConstraintValue(null, basicConstraint.address(), 
      encodedValue.address());
  NSS.lib.CERT_AddExtension(extensionHandle, 85, encodedValue.address(), 1, 1);
  ...
  NSS.lib.CERT_EncodeAndAddBitStrExtension(extensionHandle, 
      NSS.lib.SEC_OID_X509_KEY_USAGE, keyUsageItem.address(), 1);
};

CertificateManager.prototype.addVerificationDetails = function(certificate, 
    extensionHandle, verificationDetails) {
  ...
  var status = NSS.lib.CERT_EncodeAndAddBitStrExtension(
    extensionHandle, NSS.lib.SEC_OID_NS_CERT_EXT_COMMENT, 
    commentItem.address(), 0 );

  CV9BLog.pki('COMMENT v3 extension result: ' + status);
};

CertificateManager.prototype.addAltNames = function(certificate, 
    extensionHandle, altNames) {
...
CertificateManager.prototype.generateCertificate = function(privateKey, 
    publicKey, subject, issuer, isCa, altNames, verificationDetails) {
  ...
  var certificateRequest = NSS.lib.CERT_CreateCertificateRequest(subjectName, 
      publicKeyInfo, null);
  ...
  var certificate = NSS.lib.CERT_CreateCertificate(serial, issuerName, validity,
      certificateRequest);
  ...
  if (verificationDetails !== null) {
    this.addVerificationDetails(certificate, extensionHandle, 
        verificationDetails);
  }
...
CertificateManager.prototype.generateKeyPair = function(permanent, nick) {
  ...
  var rsaParams = NSS.types.PK11RSAGenParams({'keySizeInBits' : 1024,
    'pe' : 65537});
...
CertificateManager.prototype.deserialize = function(serialized) {
  this.caCertificate = Serialization.deserializeCERTCertificate(
      serialized.caCertificate);
  this.caKey = Serialization.deserializeSECKEYPrivateKey(serialized.caKey);
  this.peerKeys = {
    'publicKey'  : Serialization.deserializeSECKEYPublicKey(
        serialized.peerKeyPublic),
    'privateKey' : Serialization.deserializeSECKEYPrivateKey(
        serialized.peerKeyPrivate) 
  };
};
